...
class TestCacheKeyRedaction:
    ...
    def test_structured_log_cache_operation_without_key(self, caplog: pytest.LogCaptureFixture) -> None:
        """No ``key`` kwarg: the redaction branch is skipped and the ``unknown`` sentinel stands in."""
        with caplog.at_level(logging.INFO):
            self._orchestrator().log_cache_operation(operation="circuit_breaker_open")

        records = [r for r in caplog.records if "Cache operation: circuit_breaker_open" in r.getMessage()]
        assert records
        assert all(getattr(record, "structured", {}).get("cache_key") == "unknown" for record in records)
