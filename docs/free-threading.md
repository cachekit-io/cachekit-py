...
| `l1_cache.py` `L1CacheManager._take_over_if_forked` (LAB-4772) | Owner-PID fast path; on a PID change one thread takes over under a lock fetched by `dict.setdefault(pid, Lock())`, replaces the manager lock and stop event, replaces each `L1Cache` lock a parent thread held at fork (dropping that cache's entries, with a warning), and restarts the cleanup thread if the parent ran one | Safe. `setdefault` is atomic under per-object locking, so every thread in a new child gets the same fresh lock, which no parent thread can have held at fork; the rest block on it and re-check. The fast path gates on `_owner_pid` alone, so a weak-memory reader may still see the inherited manager lock once: the same benign fork-swap race as the SWR row, since that lock is unheld unless a parent thread held it at fork. Runs from `L1Cache.put`, never from an at-fork hook, so it also covers uWSGI; known limit: a `get()` that reaches a cache lock orphaned at fork before any `put()` in the child has run the take-over hangs for the child's life, and decorated functions call `get()` first, so their first call in that namespace is exposed |
